...
//script UNtuk Mengubah Format Rupiah
		var harga_b = document.getElementById('harga_b');
		harga_b.addEventListener('keyup', function(e){
			// tambahkan 'Rp.' pada saat form di ketik
			// gunakan fungsi formatRupiah() untuk mengubah angka yang di ketik menjadi format angka
			harga_b.value = formatRupiah(this.value, 'Rp. ');
		});
		...
		function formatRupiah(angka, prefix){
			...
			harga_b    		= split[0].substr(0, sisa),
			ribuan     		= split[0].substr(sisa).match(/\d{3}/gi);
 
			// tambahkan titik jika yang di input sudah menjadi angka ribuan
			if(ribuan){
				separator = sisa ? '.' : '';
				harga_b += separator + ribuan.join('.');
			}
 
			harga_b = split[1] != undefined ? harga_b + ',' + split[1] : harga_b;
			return prefix == undefined ? harga_b : (harga_b ? 'Rp. ' + harga_b : '');
		}
...
function resetLatLng() {
 $('input[name=lat_b').val('').reset;
 $('input[name=lng_b').val('').reset;
}


			//script untuk menampikan preview gambar
				function tampilkanPreview(gambar_b,idpreview){
                var gb = gambar_b.files;
